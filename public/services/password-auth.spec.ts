...
test("password lengths count Unicode code points and do not silently trim or truncate", () => {
  ...
  expect(validatePassword("🔑".repeat(7))).toHaveLength(1)
  expect(validatePassword("🔑".repeat(12))).toEqual([])
  expect(validatePassword("a".repeat(13))).toHaveLength(1)
  expect(validatePassword(" " + "a".repeat(6) + " ")).toEqual([])
  expect(validatePassword(" 中文密码Ab1 ")).toEqual([])
  expect(validatePassword("中文密码Ab1")).toHaveLength(1)
  expect(validatePassword("", "newPassword")).toEqual([{ field: "newPassword", message: "Use 8–12 characters. Spaces and Chinese characters are allowed." }])
})
