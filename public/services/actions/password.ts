...
export const createAccount = (username: string, name: string, password: string, role: UserRole): Promise<Result<AccountResult>> =>
  http.post("/_api/admin/accounts", { username: username.trim().toLowerCase(), name, password, role })

export const initializeAccount = (userID: number, username: string, password: string): Promise<Result<AccountResult>> =>
  http.post(`/_api/admin/accounts/${userID}/initialize`, { username: username.trim().toLowerCase(), password })

export const resetAccountPassword = (userID: number, password: string): Promise<Result<AccountResult>> =>
